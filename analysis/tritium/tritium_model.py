...
if switched_to and isinstance(switched_to, list):
    for switch in switched_to:
        ...
        if gas_switch_time:
            try:
                gas_switch_time_dt = datetime.strptime(
                    gas_switch_time, "%m/%d/%Y %H:%M"
                )
                ...
                warnings.warn(
                    f"Could not parse gas_switch_time '{gas_switch_time}': {e}"
                )
else:
...
neutron_rate = (
    (350 / 250) * 1.3e09 * ureg.neutron * ureg.s**-1
...
nb_samples_included_ov = len(OV_stream.samples)
T_produced_OV = OV_stream.get_cumulative_activity("total")[nb_samples_included_ov - 1]
